...
const Tools = (props) => {
  ...
  const btnClass = classNames("item", {
    "base-container-list": props.listView,
    "base-container-grid": props.gridView,
  });

  const getFavoriteTool = () => {
    return FAVORITE_TOOL.map((tool) => (
      <div className="radio">
        <input
  ...
  const getToolItem = () => {
    let toolsList =
      id === "" ? TOOL_DATA : TOOL_DATA.filter((item) => item.id === id);
    return toolsList.length === 0 ? (
      <label style={{ margin: "50px" }}>No Tool Avalibale</label>
    ) : (
      toolsList.map((tool) => (
        <div>
          <ToolCards
  ...
  return (
    <div>
      {/* {showViewChangeIcon()} */}
      <div className={btnClass}>{getToolItem()}</div>
    </div>
  );
};
